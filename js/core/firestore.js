 ...
 * and IDs remain telegramId-based end-to-end — this never touches
 * firebaseUid.
 */
...
/**
 * Looks up the Firestore user document (keyed by firebaseUid) whose
 * stored telegramId field matches the given Telegram ID. Needed because
 * a referral's referrerId is a telegramId, but paying the bonus requires
 * updating that user's balance at users/{firebaseUid}. Transactions can
 * only tx.get() a document reference they already know — not run a
 * query — so this resolution happens before the transaction starts.
 */
async function findUserRefByTelegramId(telegramId) {
  const q = query(collection(db, "users"), where("telegramId", "==", String(telegramId)), limit(1));
...
export async function incrementReferralTaskProgressTx(telegramId, tasksRequired, bonusCoins) {
  ...
  if (refData.bonusPaid) return refData;

  // Only bother resolving the referrer's firebaseUid if this task push
  // could actually cross the threshold.
  const wouldReachThreshold = (refData.tasksCompleted || 0) + 1 >= tasksRequired;
  const referrerDocRef = wouldReachThreshold ? await findUserRefByTelegramId(refData.referrerId) : null;
  ...
  return runTransaction(db, async (tx) => {
    ...
    if (shouldPayBonus && referrerDocRef) {
