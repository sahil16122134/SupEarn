...
export async function render(container) {
  ...
  async function loadTasks() {
   ...
   const adsgramTasks = getAdsgramTaskEntries(settings, adsgramCompleted);
    ...
    const completedCustomTasks = appState.get("appUser")?.completedCustomTasks || {};

    // Admin-managed SupEarn tasks are their own Firestore collection with
    // its own network round-trip — a failure here (permission error,
    // offline blip, missing index) must never take down the whole Tasks
    // screen when AdsGram/Adeasly still loaded fine.
    let adminTasks = [];
    try {
      const customTasks = await getTasksOnce();
      adminTasks = customTasks
      ...
      console.error("[tasks] failed to load SupEarn admin tasks:", err);
      adminTasks = [];
    }

    allTasks = [...adminTasks, ...adsgramTasks, ...adeaslyTasks];
    renderList();
  }
  ...
  openModal({
    ...
    onMount: (card) => {
      card.querySelector("#customTaskStart").onclick = async () => {
        ...
        if (task.status === "claim") {
          await creditTaskReward(task, card.querySelector("#customTaskStart"), "Claim Reward");
          return;
        }
        ...
        if (task.taskUrl) {
          ...
          task.status = "claim";
          closeModal();
          // Reflect the new "claim" status on the underlying task card
