...
export async function render(container) {
  ...
  async function loadTasks() {
    ...
    if (settings.adsgramBlockId) {
      try {
        adsgramCompleted = await isTaskAlreadyRewarded(telegramId, `adsgram_${settings.adsgramBlockId}`);
      } catch (err) {
  ...
  async function creditTaskReward(task, buttonEl, originalLabel) {
    try {
      ...
      await creditTaskRewardOnceTx(telegramId, task.id, task.reward, task.provider);

      try {
        await incrementReferralTaskProgressTx(telegramId, settings.referralTasksRequired || 4, settings.referralBonusCoins || 0);
      } catch (err) {
